  ...
  },
  {
    ...
    fiches: [
      ...
      },
      {
        ...
        tableau: { cols: ['', 'CTO', 'PEA'], rows: [['Plafond', 'Illimité', '150 000€'], ['Fiscalité', 'Flat tax 31,4%', "Après 5 ans, seulement 18,6%"], ['Produits', 'Tout (actions monde, ETF, obligations)', 'Actions et ETF européens uniquement'], ['Retrait', 'À tout moment', 'Libre mais perte avantage avant 5 ans']] },
      ...
      },
      {
        ...
        texte: "Tous les courtiers ne se valent pas. Frais, interface, gamme de produits, fiabilité. Voilà ce qui doit guider ton choix.",
        tableau: { cols: ['Plateforme', 'Type', 'Frais ETF', 'Points forts'], rows: [['Bourso Bank', 'PEA+CTO', '0% sélectionnés', 'Gratuit, fiable, banque complète'], ['Bourse Direct', 'PEA+CTO', '0,99€/ordre', 'Très faibles frais, idéal PEA'], ['Trade Republic', 'CTO', '1€/ordre', 'Ultra simple, plans auto'], ['Degiro', 'CTO', 'Dès 0,50€', 'Grande gamme, frais bas'], ['Interactive Brokers', 'CTO', 'Dès 0€', 'Pro, marchés mondiaux'], ['Linxea', 'AV', '0% UC', 'Meilleure AV en ligne']] },
        challenge: null,
        app: "Dans le Journal d'investissements, renseigne l'enveloppe, PEA ou CTO, le courtier et tes frais de courtage réels. L'app calcule automatiquement l'impact sur ta performance nette.",
      },
    ],
  },
  {
    ...
    fiches: [
      ...
      },
      {
        ...
        note: "Déclaration comptes étrangers — Formulaire 3916 obligatoire pour Degiro, Trade Republic, Interactive Brokers. Amende 1 500€ par compte non déclaré.",
        challenge: null,
    ...
    fiches: [
      {
        ...
        strategie: true,
        challenge: "Réponds aux 3 questions maintenant — horizon, montant mensuel, profil de risque. Écris tes réponses. C'est ta stratégie personnelle.",
      ...
      },
      {
        ...
        challenge: "Va sur justetf.com et recherche un ETF MSCI World éligible PEA avec un TER inférieur à 0,40%. Vérifie sa taille et son ancienneté. Si tout est bon — c'est ton candidat.",
      ...
      },
      {
        ...
        app: "Dès ton ordre passé, note-le immédiatement dans le Journal d'investissements. Ticker, quantité, montant, enveloppe, courtier et frais. C'est ton premier pas dans ton Ascension.",
      },
      {
        id: 22,
        ...
        <thead>
          <tr>
            {tableau.cols.map((col, i) => (
              <th key={i} style={{ background: t.bgSecondary, padding: '8px 12px', textAlign: 'left', fontWeight: 500, color: t.textSecondary, fontSize: 11, borderBottom: `0.5px solid ${t.border}`, whiteSpace: 'nowrap' }}>
                {col}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {tableau.rows.map((row, i) => (
            <tr key={i} style={{ background: i % 2 === 1 ? t.bgSecondary : 'transparent' }}>
              {row.map((cell, j) => (
                <td key={j} style={{ padding: '8px 12px', borderBottom: i < tableau.rows.length - 1 ? `0.5px solid ${t.border}` : 'none', color: t.text, fontSize: 12 }}>
                  {cell}
                </td>
              ))}
            </tr>
...
// ─── Section stratégie interactive (fiche 18) ────────────────────────────────
function SectionStrategie({ couleur }) {
  const t = useTheme()
  const PROFILS = [
    'Prudent — 30% ETF + 70% sécurité',
    'Équilibré — 60% ETF + 40% sécurité',
    'Dynamique — 90% ETF + 10% sécurité',
  ]
  ...
  useEffect(() => {
    const charger = async () => {
      ...
      if (data) setForm({
        ...
        profil_risque: data.profil_risque || '',
        notes: data.notes || '',
      })
    }
    charger()
  ...
  const sauvegarder = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        montant_mensuel: form.montant_mensuel,
        profil_risque: form.profil_risque,
        notes: form.notes,
        updated_at: new Date().toISOString(),
      }
      const { data: existing } = await supabase.from('strategies').select('id').eq('user_id', user.id).single()
      const { error } = existing
        ? await supabase.from('strategies').update(payload).eq('user_id', user.id)
  ...
  return (
    <div style={{ background: t.bgSecondary, border: `0.5px solid ${couleur}25`, borderRadius: 10, padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {/* Question 1 — Horizon */}
      <div>
        <div style={{ fontSize: 11, fontWeight: 500, color: t.textSecondary, marginBottom: 6 }}>
          1. Sur quel horizon souhaites-tu investir ?
        </div>
      ...
      {/* Question 2 — Montant mensuel */}
      <div>
        <div style={{ fontSize: 11, fontWeight: 500, color: t.textSecondary, marginBottom: 6 }}>
          2. Quel montant mensuel peux-tu investir ?
        </div>
        <input
          type="text"
      ...
      {/* Question 3 — Profil de risque */}
      <div>
        <div style={{ fontSize: 11, fontWeight: 500, color: t.textSecondary, marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {PROFILS.map((profil) => (
            <div
              key={profil}
              onClick={() => setForm({ ...form, profil_risque: profil })}
              style={{
                padding: '10px 14px',
                borderRadius: 8,
                border: `0.5px solid ${form.profil_risque === profil ? couleur : t.border}`,
                background: form.profil_risque === profil ? couleur + '12' : t.bgCard,
                cursor: 'pointer',
                fontSize: 12,
                color: form.profil_risque === profil ? couleur : t.text,
                fontWeight: form.profil_risque === profil ? 500 : 400,
      ...
      <div>
        ...
        <textarea
          placeholder="Ex: Je veux investir dans un ETF World en PEA chez Bourso Bank..."
          value={form.notes}
      ...
      {erreur && (
        <div style={{ background: '#FCEBEB', border: '0.5px solid #E24B4A', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: '#E24B4A' }}>
          ⚠️ {erreur}
        </div>
      )}

      {saved && (
        <div style={{ background: '#EAF6E4', border: '0.5px solid rgba(76,175,46,0.25)', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: '#2E7D1E' }}>
      ...
      <button
        onClick={sauvegarder}
        disabled={loading}
        style={{ background: loading ? '#9CA3AF' : '#4CAF2E', color: '#fff', border: 'none', borderRadius: 8, padding: '10px', fontSize: 13, fontWeight: 500, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}
      >
...
// ─── Popup fiche ─────────────────────────────────────────────────────────────
function PopupFiche({ fiche, chapitre, onClose }) {
  const t = useTheme()
    ...
    >
      <div style={{ background: t.bgCard, borderRadius: 16, border: `0.5px solid ${t.border}`, width: '100%', maxWidth: 620, maxHeight: '88vh', overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {fiche.note && (
            <div style={{ background: t.bgSecondary, border: `0.5px solid ${t.border}`, borderRadius: 8, padding: '10px 12px', fontSize: 12, color: t.textSecondary, lineHeight: 1.5 }}>
              {fiche.note}
            </div>
          )}

          {fiche.citation && (
            <div style={{ borderLeft: `3px solid ${chapitre.couleur}`, padding: '10px 14px', fontSize: 13, color: t.textSecondary, fontStyle: 'italic', lineHeight: 1.6 }}>
              "{fiche.citation}"
            </div>
          )}

          {/* Section stratégie interactive uniquement pour la fiche 18 */}
...
export default function Guide() {
  ...
  const ouvrirFiche = (fiche, chapitre) => {
    setFicheOuverte(fiche)
    setChapitreOuvert(chapitre)
  }
  ...
  return (
    <div style={{ background: t.bg, minHeight: '100vh', display: 'flex', flexDirection: 'column', overflowX: 'hidden', width: '100%' }}>
      {ficheOuverte && <PopupFiche fiche={ficheOuverte} chapitre={chapitreOuvert} onClose={fermerFiche} />}

      <Navbar page="Guide" />

      <div style={{ padding: '16px', flex: 1, display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 1000, margin: '0 auto', width: '100%', boxSizing: 'border-box' }}>
  ...
  <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.65)' }}>
    23 fiches pour comprendre, agir et ne pas se faire piéger
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {CHAPITRES.map((ch) => (
            <div key={ch.num} style={{ background: t.bgCard, border: `0.5px solid ${t.border}`, borderRadius: 12, overflow: 'hidden' }}>
              ...
              <div style={{ borderTop: `0.5px solid ${t.border}` }}>
                {ch.fiches.map((fiche, idx) => (
                  ...
                  >
                    <span style={{ fontSize: 10, fontWeight: 500, color: ch.couleur, minWidth: 22 }}>
                      {String(fiche.id).padStart(2, '0')}
                    </span>
